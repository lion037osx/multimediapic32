...
static void I2C1Idle(void);
static BOOL I2C1Start(void);
static void I2C1Idle(void);
static void I2C1Stop(void);
static BOOL I2C1SendByte(BYTE data);
 ...
 */
BOOL drvI2C1ReadRegisters(UINT8 reg, UINT8* rxPtr, UINT8 len, UINT8 slave_adr) {
 ...
 * @return Boolean indicating if operation completed successfully or not
 */
BOOL drvI2C1WriteRegisters(UINT8 adr, UINT8* data, UINT8 len, UINT8 slave_adr) {
    UINT8 i, flag, j;
    flag = 0;
    for (i = 0; i < 100; i++) {
        //1. i2c start
 ...
 * @return Boolean indicating if operation completed successfully or not
 */
BOOL drvI2C1WriteByte(UINT8 reg, UINT8 byte, UINT8 slave_adr) {
    return ( drvI2C1WriteRegisters(reg, &byte, 1, slave_adr));
}
