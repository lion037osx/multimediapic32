...
if((day&OSCON)==OSCON)return;            // if oscillator = already running, do nothing. 
	...
	WRITE_RTCC(ADDR_MNTH,0x01);   	     // initialize MONTH register : november   
	WRITE_RTCC(ADDR_DATE,0x01);   	     // initialize DATE  register : date = 01  
	WRITE_RTCC(ADDR_HOUR,0x00);   	     // initialize HOUR  register : hour = 00  
	WRITE_RTCC(ADDR_MIN,0x00) ;   	     // initialize MIN   register : min  = 00  
